...
BundleLocator.prototype = {
     ...
     * @param {Function} filter optional function to execute on each bundle. The
     *   function will receive the following arguments:
     *     @param {String} filter.bundleName the current bundle name
     *     @param {Object} filter.packageJSON the parsed package.json contents
    ...
    listBundleNames: function (filter) {
        ...
        for (bundleName in bundles) {
            if (bundles.hasOwnProperty(bundleName)) {
                ...
                try {
                    pkgJSON = require(libpath.resolve(bundle.baseDirectory, 'package.json'));
                } catch (e) {
                    // if no package.json found, tall the filter so!
                    pkgJSON = undefined;
                }
                if (filter(bundle, pkgJSON)) {
