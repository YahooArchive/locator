...
BundleLocator.prototype = {
    ...
    parseBundle: function (dir, options) {
        var self = this;
        return this._walkNPMTree(dir).then(function (pkgs) {
            pkgs = self._filterNPMPackages(pkgs);
            var promises = [];
            pkgs.forEach(function (pkg) {
                var opts = (pkg.dir === dir) ? options : {};
                promises.push(self._walkPackage(pkg, opts));
            });
    ...
    _makeBundle: function (path, options) {
        ...
        return new libpromise.Promise(function (fulfill, reject) {
            ...
            try {
                // need to do this for non-NPM bundles
    ...
    _processFileQueue: function () {
        ...
        return new libpromise.Promise(function (fulfill, reject) {
            ...
            // TODO -- handle the following situations
            //  * bundle not found (signifies an added bundle?)
            //  * bundle deleted during watch (how?)
            //  * bundle added during watch (how?)
            //  * relativePath starts with "node_modules"
    ...
    /**
     * Walks a directory and returns a list of metadata about modown packages
     ...
     *      <dd> The version of the package </dd>
     *      <dt> depth {integer} </dt>
     *      <dd> The depth in the NPM package dependency tree </dd>
     *      <dt> options {object} </dt>
     *      <dd> Options for the package, i.e. the `"modown"` section of `package.json` </dd>
     *  </dl>
     * @private
     * @method _walkNPMTree
     * @async
     ...
     * about modown packages in the directory.
     */
                        ...
                        if (subpkgsResults && Array.isArray(subpkgsResults)) {
                            subpkgsResults.forEach(function (subpkgResults) {
                                if (subpkgResults && subpkgResults.length) {
                                    pkgs = pkgs.concat(subpkgResults);
                                }
                            });
                        }
                        fulfill(pkgs);
    ...
    /**
     * Figures out which packages to use from the list.
     * The returned list is sorted first by depth then by package name.
     * This method works especially well with the output of _walkNPMTree().
     * @private
     * @method _filterNPMPackages
     * @param {array} all List of all the package metas to consider.
     * @return {array} The metas of the packages to actually use.
     */
    _filterNPMPackages: function (all) {
        ...
        all.forEach(function (pkg) {
            ...
            if (!byDepth[pkg.name][pkg.depth]) {
                byDepth[pkg.name][pkg.depth] = [];
            }
            byDepth[pkg.name][pkg.depth].push(pkg);
        });
        Object.keys(byDepth).forEach(function (name) {
            ...
            pkgs = pkgDepths[minDepth];
            if (1 === pkgs.length) {
                if (minDepth !== maxDepth) {
                    imports.log('multiple "' + name + '" packages found, using ' + pkgs[0].dir);
                }
                out.push(pkgs[0]);
                return;
            }
     ...
     * @private
     * @method _walkPackage
     * @param {object} pkg Metadata about the package. See the docs for _walkNPMTree()
     * for format of this metadata.
     * @return {Promise/A+} A promise that will be fulfilled with the bundle
     * made from the NPM package.
     */
    _walkPackage: function (pkg) {
        var self = this;
        // TODO -- merge options (second arg) into pkg.options
        return this._makeBundle(pkg.dir, pkg.options).then(function (bundle) {
            var parentName;

            self._bundles[bundle.name] = bundle;

            // wire into parent
            parentName = self._getBundleNameByPath(libpath.dirname(bundle.baseDirectory));
            if (parentName) {
                if (!self._bundles[parentName].bundles) {
                    self._bundles[parentName].bundles = {};
                }
                self._bundles[parentName].bundles[bundle.name] = bundle;
            }

            return new libpromise.Promise(function (fulfill) {
                var walker = libtromp(bundle.baseDirectory);
                walker.reject(/^node_modules\b/);
                walker.on('file', function (entry) {
                    entry = entry.toString();
                    if (!self._isBuildFile(entry)) {
                        self._fileQueue.push([EVENT_UPDATED, entry]);
                    }
                });
                walker.on('dir', function (entry) {
                    entry = entry.toString();
                    if (!self._isBuildFile(entry)) {
                        self._fileQueue.push([EVENT_UPDATED, entry]);
                    }
