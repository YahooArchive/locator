...
BundleLocator.prototype = {
     ...
     *      <dd> Options for the bundle, taken in part from the `"modown"` section of `package.json` </dd>
     *  </dl>
     * @private
     * @method _makeBundleSeed
     * @param {string} dir Full path to the bundle.
     ...
     */
    _makeBundleSeed: function (dir, name, version, pkg, options) {
        var seed;
        seed = {
            baseDirectory: dir,
            name: name,
     ...
     * comes from "modown" in package.json.
        ...
        return new libpromise.Promise(function (fulfill, reject) {
            ...
            if (ruleset._bundles) {
                ...
                if (subBundleSeed) {
                    ...
                    return self._makeBundle(subBundleSeed, bundle).then(fulfill, function (err) {
                        reject(err);
                    });
                }
            }
     ...
     * @private
     * @method _loadRuleset
     * @param {Bundle|BundleSeed} bundle The bundle (or bundle seed, seed _makeBundleSeed())
    ...
    _loadRuleset: function (bundle) {
        ...
        if (bundle.options && bundle.options.rulesets) {
            try {
                rulesets = require(libpath.resolve(bundle.baseDirectory, bundle.options.rulesets));
            } catch (errLocal) {
            ...
            if (!rulesets) {
                ...
                while (dir) {
                    try {
                        rulesets = require(libpath.resolve(dir, bundle.options.rulesets));
                        break;
                    } catch (errDir) {
                    ...
                    try {
                        rulesets = require(libpath.resolve(dir, 'node_modules', bundle.options.rulesets));
                        break;
                    } catch (errDep) {
