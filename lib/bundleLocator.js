...
var libdive = require('dive'),
    ...
    DEFAULT_SELECTOR = '{}',
    EVENT_FILE_ADD = 'resourceAdded',
    EVENT_BUNDLE_ADD = 'bundleAdded';
                    ...
                    if (rule.subtypeKey) {
                        Object.keys(bundle.resources[selector][resType]).forEach(function(subtype) {
                            Object.keys(bundle.resources[selector][resType][subtype]).forEach(function(name) {
                                var fullPath,
                                    res;
                                fullPath = bundle.resources[selector][resType][subtype][name];
                                res = {
                                    selector: selector,
                                    type: resType,
                                    subtype: subtype,
                        ...
                        Object.keys(bundle.resources[selector][resType]).forEach(function(name) {
                            var fullPath,
                                res;
                            fullPath = bundle.resources[selector][resType][name];
                            res = {
                                selector: selector,
                                type: resType,
                                fullPath: fullPath,
                    ...
                    if (err) {
                        ...
                        libfs.writeFile(path, contents, options, function(err) {
                            if (err) {
                                ...
                                self._fileQueue.push([EVENT_FILE_ADD, path]);
                                fulfill();
                            }
                        });
                    }
...
BundleLocator.prototype = {
    ...
    _processFileQueue: function() {
        ...
        return new libpromise.Promise(function(fulfill, reject) {
            ...
            // This will be augmented if a rule matches.
            res = {
            ...
            self._onResource(res, eventName).then(fulfill, reject);
        }).then(function() {
    ...
    _processBundleUpdates: function() {
        ...
        Object.keys(this._bundleUpdates).forEach(function(bundleName) {
            ...
            delete self._bundleUpdates[bundleName];
            self._plugins.forEach(function(plugin) {
                if (plugin.plugin[EVENT_BUNDLE_ADD]) {
                    pluginPromises.push(new libpromise.Promise(function(fulfill, reject) {
                        var ret = plugin.plugin[EVENT_BUNDLE_ADD](bundle, self._pluginAPI);
        ...
        return libpromise.batch.apply(libpromise, bundlePromises).then(function() {
            // catch changes to bundles that might have happened while processing the bundles
            if (Object.keys(self._bundleUpdates).length) {
                return self._processBundleUpdates();
            }
        });
    },
     ...
     * @param {string} eventName What is happening to the resource.
     * @return {Promise/A+} A promise that will be fulfilled once
     * the resource is handled.
     */
    _onResource: function(res, eventName) {
        var self = this,
            bundle = self._bundles[res.bundleName],
            type = res.type,
            subtype = res.subtype,
        ...
        if (res.type) {
            ...
            this._bundleUpdates[res.bundleName] = true;
        }

        this._plugins.forEach(function(plugin) {
            if (self._filterResource(res, plugin.filter) && plugin.plugin[eventName]) {
                promises.push(new libpromise.Promise(function(fulfill, reject) {
                    var ret = plugin.plugin[eventName](res, self._pluginAPI);
                    if (libpromise.Promise.isPromise(ret)) {
                        ret.then(fulfill, reject);
                    } else {
                        fulfill();
