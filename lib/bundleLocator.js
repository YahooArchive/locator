...
var libfs       = require('fs'),
    ...
    PATH_SEP_REGEX = new RegExp(PATH_SEP === '\\' ? ('\\' + PATH_SEP) : PATH_SEP, 'g');
...
BundleLocator.prototype = {
    ...
    _ruleBundles: function (fullPath, relativePath, rule, parent) {
        ...
        for (r = 0; r < rule.length; r += 1) {
            ...
            if (matches) {
                ...
                match = matches[1];
                if ('/' !== separator) {
                    match = libpath.normalize(match);
                }
                return this._makeBundleSeed(fullPath, match, defaultVersion, pkg, rule[r].options);
            }
        }
    },
    ...
    _onFile: function (res, ruleset, eventType) {
        ...
        return libpromise.batch.apply(libpromise, promises).then(function () {
            ...
            if (PATH_SEP !== '/') {
                relativePath = relativePath.replace(PATH_SEP_REGEX, '/');
            }
