...
BundleLocator.prototype = {
     ...
     * Anything in a bundle's node_modules subdir is ignored.
     * @private
     * @method _walkBundle
    ...
    _walkBundle: function (bundleSeed) {
        ...
        function makeBundleResolver(bundle) {
            ...
            function typer(err, path) {
                var subpath = path.slice(bundle.baseDirectory.length),
                    is_dot = '.' === libpath.basename(path).substr(0, 1),
                    is_pkg = -1 !== subpath.indexOf('node_modules'),
                    ignore = is_dot || is_pkg || self._isBuildFile(path);

                return ignore && 'ignored';
            }

            function queue(err, path) {
                self._fileQueue.push([EVENT_UPDATED, path]);
            }

            return new libpromise.Promise(function (fulfill, reject) {
                var walker = new ScanFS([], typer);

                function afterwalk(err) {
                    return err ? reject(err) : fulfill(bundle);
                }

                self._bundles[bundle.name] = bundle;
                walker.on('file', queue);
                walker.on('dir', queue);
                walker.on('done', afterwalk);
